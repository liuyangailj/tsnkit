...
class FlowGraphDataset:
    def __init__(self, flows, topology, max_period=None):
        """
        Args:
            flows: list of flow dicts (from FlowGenerator)
            topology: networkx graph (for link info)
    ...
    def get_node_features(self):
        ...
        特征包含: [Norm_Period, Norm_Size, Norm_Deadline]
        """
        features = []
        for f in self.flows:
            # 这里简化特征：周期、大小、Deadline
            # 未来可以把 KSP 的链路 ID embedding 加进来，但先跑通基础的
            x = [
                self._normalize(f['period'], self.max_period),
                self._normalize(f['size'], self.max_size),
                self._normalize(f['deadline'], self.max_period) # 假设 deadline 跟周期在一个量级
            ]
            features.append(x)
